...
// (Optional) wire up your services when ready
// import { InventoryService, StoreService } from "@/lib/services/InventoryService";

/**
 * AddBranchWizard
 * A multi-step onboarding and inventory import flow for adding a new branch,
 * inspired by a franchise-style application.
 */
export default function AddBranchWizard({ onDone }: { onDone?: () => void }) {
  ...
  function guessMapping(hs: string[]) {
    const m: Record<FieldKey, string | ""> = { sku: "", brand: "", model: "", size: "", loadIndex: "", dotCode: "", qty: "", basePrice: "", promoPrice: "" };
    const find = (cands: string[]) => hs.find((h) => cands.includes(h.toLowerCase().trim()));
    const lower = hs.map((h) => h.toLowerCase().trim());
    const at = (name: string) => hs[lower.indexOf(name)] ?? "";
    m.brand = find(["brand"]) || at("brand");
    m.model = find(["model"]) || at("model");
  ...
  const mappedRows: ImportRow[] = useMemo(() => {
    ...
    return rows.map((r) => {
      ...
      if ((promo === "" || promo === undefined) && treatEmptyPromoAsNull) promoVal = null;
      return {
        sku: get("sku"),
  ...
  const mappingValid = useMemo(() => {
    ...
    return REQUIRED_FIELDS.every((k) => mapping[k] && headers.includes(mapping[k] as string));
  }, [headers, mapping]);
  ...
  function downloadTemplate() {
    const cols = [
      ...
      "BRIDGESTONE,TURANZA T005A,225/45R17,91W,2319,2,3700,,-",
    ].join("\n");
  ...
  async function importToInventory() {
    ...
    const valid = mappedRows.filter((r) => r.brand && r.model && r.size && r.dotCode && r.qty > 0);
    if (!valid.length) return toast.error("No valid rows to import");

    // Example sequential import. Replace with your InventoryService wiring.
    await toast.promise(
      (async () => {
        for (const r of valid) {
          // Example pseudo-code (uncomment & wire up in your codebase):
          // const { brandId } = autoCreate
          //   ? await InventoryService.ensureBrandDoc(biz.businessName /* or branchId */, r.brand)
          //   : await InventoryService.getBrandId(/*...*/);
          // const { modelId } = autoCreate
          //   ? await InventoryService.ensureModelDoc(/*storeId*/ biz.businessName, brandId, r.model)
          //   : await InventoryService.getModelId(/*...*/);
          // const variantId = `${r.size.replace(/\s+/g, '').toLowerCase()}${r.loadIndex ? `-${r.loadIndex.replace(/\s+/g, '').toLowerCase()}` : ''}`;
          // if (autoCreate) {
          //   await InventoryService.ensureVariantPath(/*storeId*/ biz.businessName, brandId, modelId, variantId, {
          //     size: r.size,
          //     loadIndex: r.loadIndex,
          //     basePrice: r.basePrice,
          //   });
          // }
          // await InventoryService.addNewDot(/*storeId*/ biz.businessName, brandId, modelId, variantId, {
          //   dotCode: r.dotCode,
          //   qty: r.qty,
      ...
      { loading: "Importing inventory...", success: "Import complete", error: "Import failed" }
    );
    next();
  }
  ...
  async function submitApplication() {
    // Save business info + finalize onboarding
    await toast.promise(
      (async () => {
        // Example: await StoreService.createBranchApplication(biz)
  ...
  function StepBusinessInfo() {
    return (
      <Card className="rounded-xl">
        <CardHeader>
          <CardTitle className="text-lg flex items-center gap-2"><Building2 className="h-5 w-5"/>Business Information</CardTitle>
        ...
        <CardContent className="space-y-5">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div>
              ...
              <Select value={biz.inventoryValue} onValueChange={(v:any)=>setBiz({...biz,inventoryValue:v})}>
                <SelectTrigger className="mt-1"><SelectValue placeholder="Select inventory value"/></SelectTrigger>
                <SelectContent>
          ...
          <div className="flex items-start gap-2 text-xs text-muted-foreground mt-1">
            ...
            <div>
              All information provided will be kept confidential and used only for network verification purposes.
            </div>
          </div>

          <div className="flex items-center justify-between pt-2">
            <div className="text-xs text-muted-foreground flex items-center gap-1"><Info className="h-3.5 w-3.5"/> You can import inventory in the next step.</div>
            <Button onClick={next} disabled={!bizValid}>
              Continue
            </Button>
          </div>
        </CardContent>
  ...
  function StepImport() {
    return (
      <Card className="rounded-xl">
        <CardHeader>
          <CardTitle className="text-lg flex items-center gap-2"><FileSpreadsheet className="h-5 w-5"/>Import Inventory (CSV)</CardTitle>
          <CardDescription>Upload your stock list and map columns. You can skip and import later.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-5">
          <div className="flex flex-wrap items-center gap-2">
            <Input ref={csvInputRef} type="file" accept=".csv" className="hidden" onChange={(e)=>{const f=e.target.files?.[0]; if(f) parseCSV(f);}} />
            <Button variant="outline" onClick={handleChooseFile} className="gap-2"><UploadCloud className="h-4 w-4"/>Choose CSV</Button>
            <Button variant="ghost" onClick={downloadTemplate} className="gap-2"><FileDown className="h-4 w-4"/>Download template</Button>
          ...
          {headers.length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {[...REQUIRED_FIELDS, ...OPTIONAL_FIELDS].map((f) => (
                <div key={f}>
                  <Label className="text-xs">{f}{REQUIRED_FIELDS.includes(f as any) && " *"}</Label>
          ...
          {mappedRows.length > 0 && (
            <div className="rounded-md border overflow-hidden">
              <div className="px-3 py-2 text-xs bg-slate-50 flex items-center justify-between">
                <div>Preview ({Math.min(10, mappedRows.length)} of {mappedRows.length} rows)</div>
                <div className="text-muted-foreground">{mappingValid ? "Mapping complete" : "Please map required fields"}</div>
              </div>
                  ...
                  <thead className="bg-slate-50">
                    <tr>
                      {(["sku","brand","model","size","loadIndex","dotCode","qty","basePrice","promoPrice"]) .map((k)=> (
                        <th key={k} className="px-3 py-2 text-left font-medium text-slate-600 border-b">{k}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
          ...
          <div className="flex items-center justify-between">
            <Button variant="ghost" className="gap-1" onClick={prev}><ArrowLeft className="h-4 w-4"/>Back</Button>
        ...
        <CardContent className="space-y-4">
          <div className="rounded-md border">
            ...
            <div className="p-3 text-sm grid grid-cols-1 md:grid-cols-2 gap-y-1">
              <div><span className="text-slate-500">Name:</span> {biz.businessName || '-'}</div>
              <div><span className="text-slate-500">Type:</span> {biz.businessType || '-'}</div>
              <div><span className="text-slate-500">Location:</span> {biz.location || '-'}</div>
              <div><span className="text-slate-500">Established:</span> {biz.establishedYear || '-'}</div>
              <div><span className="text-slate-500">Revenue:</span> {biz.revenueRange || '-'}</div>
              <div><span className="text-slate-500">Inventory Value:</span> {biz.inventoryValue || '-'}</div>
              <div className="md:col-span-2"><span className="text-slate-500">Challenges:</span> {biz.challenges || '-'}</div>
              <div className="md:col-span-2"><span className="text-slate-500">Goals:</span> {biz.goals || '-'}</div>
              <div><span className="text-slate-500">Contact:</span> {biz.contactName || '-'}</div>
              <div><span className="text-slate-500">Email:</span> {biz.email || '-'}</div>
            </div>
          </div>
          ...
          <div className="rounded-md border">
            ...
            <div className="p-3 text-sm flex items-center justify-between">
              <div className="text-muted-foreground">{mappedRows.length ? `${mappedRows.length} row(s) prepared` : 'No CSV imported (you can import later).'} </div>
              <Button variant="ghost" className="gap-1" onClick={()=>setStep(1)}><FileSpreadsheet className="h-4 w-4"/>Edit</Button>
            </div>
          </div>

          <div className="flex items-center justify-between">
            <Button variant="ghost" className="gap-1" onClick={prev}><ArrowLeft className="h-4 w-4"/>Back</Button>
            <Button onClick={submitApplication} className="gap-2"><ArrowRight className="h-4 w-4"/>Submit Application</Button>
          </div>
        </CardContent>
      </Card>
    );
  ...
  function StepDone() {
    return (
      <Card className="rounded-xl">
        <CardHeader>
          <CardTitle className="text-lg flex items-center gap-2"><CheckCircle2 className="h-5 w-5 text-green-600"/>Application Submitted</CardTitle>
          <CardDescription>We will verify your business and activate the branch shortly.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          <p className="text-sm text-muted-foreground">You can track your application status on this page. Once approved, you'll be able to transfer and receive stock through the network.</p>
          <div className="flex gap-2">
            <Button onClick={()=> onDone?.()}>Go to Dashboard</Button>
            <Button variant="outline" onClick={()=> setStep(1)}>Import more stock</Button>
          </div>
        </CardContent>
      </Card>
    );
