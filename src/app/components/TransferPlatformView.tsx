...
export default function TransferPlatformView({
  ...
  const [viewMode, setViewMode] = useState<'table' | 'grid'>(
    () => (typeof window !== 'undefined' ? ((localStorage.getItem('tp:view') as any) || 'table') : 'table')
  );
      ...
      .flatMap((size: any) =>
        (size.dots ?? []).map((dot: any) => {
          const cartItem = cart.find(
            (c) => c.dotCode === dot.dotCode && c.branchId === branch.branchId && c.variantId === String(size.variantId ?? '')
          );
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="text-center">
          <span
            className={`font-medium ${row.original.available === 0 ? 'text-red-600' : row.original.available < 6 ? 'text-amber-600' : 'text-green-600'}`}
          >
        ...
        <Card className="rounded-xl border bg-white overflow-hidden shadow-sm">
          ...
          <CardContent className="p-0">
            <div className="border-0">
              {/* Table Header */}
              <div className="grid gap-4 px-4 py-3 text-xs font-medium text-muted-foreground bg-muted/30 border-b sticky top-0 z-10">
                      ...
                      >
                        <div className="border-b hover:bg-muted/20 transition-colors">
                          ...
                          <div style={{ gridTemplateColumns: '40px 1fr 160px 80px 80px 120px 100px' }} className="grid gap-4 px-4 py-3 items-center">
                            ...
                            <div className="text-center">
                              <div className={`text-lg font-bold ${rowData.available === 0 ? 'text-red-600' : rowData.available < 6 ? 'text-amber-600' : 'text-green-600'}`}>
                                {rowData.available}
                              </div>
                                          ...
                                          <div key={`${dot.dotCode}-${idx}`} className="flex items-center justify-between p-2 bg-white rounded border hover:border-blue-200 transition-colors">
                                            ...
                                            <div className="flex items-center gap-3">
                                              ...
                                              <div className="text-right">
                                                <div className={`text-sm font-medium ${dot.hasPromo ? 'text-green-600' : ''}`}>{formatTHB(dot.price)}</div>
